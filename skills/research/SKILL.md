...
2. Ask a scope, focus, or depth question only when the answer changes what you investigate. A tight, factual query needs none.
3. Search broadly, then read narrowly. Prefer `rg`, `rg --files`, repository history, tests, configs, and neighboring implementations.
4. Use subagents for independent research tracks when available and useful: one for code path discovery, one for tests/contracts, one for history/prior decisions. The read-only `investigator` agent fits these tracks when the host supports named subagents; follow `delegating` to brief and size them. Ask for file references and evidence, not opinions.
