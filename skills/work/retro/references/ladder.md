...
Load when routing retro findings. Each finding goes on the **lowest rung that fully removes the
attention** — the cheapest thing that means nobody has to think about it again.

Going too high is the common failure: a skill proposed where a one-line rule would do, a script
written where a lint rule already exists. Going too low is rarer and shows up as a rule nobody
follows.
...
- Name the rung below the one you chose and say why it is not enough. A proposal that cannot
  explain that is probably one rung too high.
- Prefer removing the need over automating the need. Rung 0 is a real answer and the best one.
- One finding, one rung. If it needs two mechanisms, it is two findings.
...
one document, or if the document does not exist, writing it once (rung 0's cousin — do the work
